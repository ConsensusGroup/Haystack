...
class Dynamic_Public_Ledger(Configuration, User_Profile):
	...
	def Validate_User(self, Ledger_Entry, ScanAll = False):
		...
		elif Decryption().SignatureVerification(ToVerify = ToVerify, PublicKey = PublicKey, Signature = Signature).Verified == True and [Submited[0],PublicKey] not in self.Ledger_Accounts and ScanAll == True:
