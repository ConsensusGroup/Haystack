...
class Initialization(Configuration, Tools):
	...
	def InboxGenerator(self, Output_Directory = False):
		self.MessageRoot = self.Build_Directory(directory = str(self.UserFolder+"/"+self.MessageFolder), Return_Dir = Output_Directory)
		self.ReceivedMessages = self.Build_Directory(directory = str(self.UserFolder+"/"+self.MessageFolder+"/"+self.ReceivedMessages), Return_Dir = Output_Directory)
		self.RelayedMessages = self.Build_Directory(directory = str(self.UserFolder+"/"+self.MessageFolder+"/"+self.RelayedMessage), Return_Dir = Output_Directory)
		self.OutstandingRelay = self.Build_Directory(directory = str(self.UserFolder+"/"+self.MessageFolder+"/"+self.NotRelayedMessage), Return_Dir = Output_Directory)
		return self
