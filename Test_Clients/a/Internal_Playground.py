from DynamicPublicLedger_Module import Dynamic_Public_Ledger
from Haystack_Module import Sender_Client
from Inbox_Module import Trusted_Paths
from IOTA_Module import *
from User_Modules import User_Profile

DLP_Accounts = [['JAXTITIGBOXUMLOEDXP9XPJRFXCXZZUDVVSGHZJQWTZPMRLWKIFWOBEEZVRPN9Y9YAZVAK9CNFDGXADED', '-----BEGIN PUBLIC KEY-----\\nIQIDAQAB\n-----END PUBLIC KEY-----'], ['FFZLNZUOXYPWGUFPAEKOZBAYSVIOSCSSIGXLGYFWFUKGVJONKFSRMGAPUV9TWNPFQCFPKDO9HXTIWUFCZ', '-----BEGIN PUBLIC KEY-----\\nfwIDAQAB\n-----END PUBLIC KEY-----'], ['CBDDMNTYRDZCXUDCQYHABILVXA9JAQBMQIEPMIDVWLBX9A9FCGKJOMUE9ODDGTPIWHNZKVWJAKMJRFRTC', '-----BEGIN PUBLIC KEY-----\\nwwIDAQAB\n-----END PUBLIC KEY-----'], ['AWSQFWZKSJX9MVEXQMN9E9FI9PJGYAWJVOYBGYNNSL9QFOCNWXAPUZDYBBIXJMZZNRLZENZINMPOU9J9D', '-----BEGIN PUBLIC KEY-----\\nywIDAQAB\n-----END PUBLIC KEY-----'], ['ZZWUODLJFBHDUEILOFPPTLFGFZQAFOJQMIEZRPLBFZIWTVFOALOMRBXXSPLZYOPFCVVCAPWZTTBEHGPRZ', '-----BEGIN PUBLIC KEY-----\\nCwIDAQAB\n-----END PUBLIC KEY-----']]


def Test_DPL():
	DLP = Dynamic_Public_Ledger()
	DLP.Start_Ledger()
	print(DLP.Check_User_In_Ledger().Ledger_Accounts)
	print("#####")
	print(DLP.Check_User_In_Ledger(Current_Ledger = True).Ledger_Accounts)

def Test_Sender(Message, DLP_Accounts):
    Sender_Client().Send_Message(Message = Message, ReceiverAddress = DLP_Accounts[0][0], PublicKey = DLP_Accounts[0][1], DifferentPaths = 1)

def Test_Paths():
    x = Trusted_Paths()
    x.Catch_Up()

def Test_Trajectory():
    x = Trusted_Paths()
    y = Sender_Client().Ping_Function()
    x.Scan_Paths()

def Play():
	for i in range(15):
		print(IOTA_Module(Seed = User_Profile().Private_Seed).Generate_Address(Index = Dynamic_Public_Ledger().Calculate_Block().Block+i))
		print(IOTA_Module(Seed = User_Profile().Private_Seed).Generate_Address(Index = Dynamic_Public_Ledger().Calculate_Block().Block-i))

if __name__ == "__main__":
    #Test_Paths()
    #Test_DPL()
    #Test_Sender(Message = "This is the final test of removing. With multiple paths on>>>>>", DLP_Accounts = DLP_Accounts)
    #Test_Trajectory()
	#Play()
