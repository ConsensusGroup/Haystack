from DynamicPublicLedger_Module import Dynamic_Public_Ledger
from Haystack_Module import Sender_Client
from Inbox_Module import Trusted_Paths

DLP_Accounts = [['9JPHXJVPQCWEANUROGYOKVMFAHEMJKPLCUIONZFXGQQMJESIOUFDHYXRACPYZPWLJF9OXOGYSUQQFTSGC', '-----BEGIN PUBLIC KEY-----\\nNQIDAQAB\n-----END PUBLIC KEY-----'], ['XBDOAXLKAVKTTUKYQIQQLVFIEZKEBRPCRDFUWUYCBEITICSLTCAQPAIXC9CRBPXTCUAZDQEJHTYPNHFMC', '-----BEGIN PUBLIC KEY-----\\niwIDAQAB\n-----END PUBLIC KEY-----'], ['ZWTJNP9DWA9NBYEILESGBJNMUM9GCP9FRVOTZXZC9FWDOQILEFADGJBVDRB9L9HUULJNZAQOSRUPVNOHB', '-----BEGIN PUBLIC KEY-----\\nqwIDAQAB\n-----END PUBLIC KEY-----'], ['LIQQBPA9EXOISWFIFLLQCSVNUUXUDYSFXBXKVHEXNHGOEVOYLLNBOZXMUHTNZKINZNHLXZOWDN9CHJIVW', '-----BEGIN PUBLIC KEY-----\\nOwIDAQAB\n-----END PUBLIC KEY-----'], ['FBWLDAMLMUWGWHYCJLELZFKSD9DBLOFOUUKMIPJYJNOZSIOQDUROOZBYHSARQLLRWVRQ9HEKYRFFKVJSD', '-----BEGIN PUBLIC KEY-----\\n0wIDAQAB\n-----END PUBLIC KEY-----']]

def Test_DPL():
    DLP = Dynamic_Public_Ledger().Start_Ledger()
    print(DLP.Ledger_Accounts)

def Test_Sender(Message, DLP_Accounts):
    print(Sender_Client().Send_Message(Message = Message, ReceiverAddress = DLP_Accounts[0][0], PublicKey = DLP_Accounts[0][1], DifferentPaths = 1))

def Test_Paths():
    x = Trusted_Paths()
    x.Catch_Up()

def Test_Trajectory():
    x = Trusted_Paths()
    y = Sender_Client().Ping_Function()

if __name__ == "__main__":
    #Test_Paths()
    #Test_DPL()
    Test_Sender(Message = "This is another test of the DLP LOLLOLOLOL>>>>>", DLP_Accounts = DLP_Accounts)
    #Test_Trajectory()


    #Note: comment line 34 in Haystack_Module.py
