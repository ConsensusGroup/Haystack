...
def normalise(plaintext):
    normaltext = str(plaintext) + (default_size - len(plaintext) - 4) * str(' ') + '////'
    return normaltext

def split(input):
	return [input[start:start+size] for start in range(0, len(input), default_size)]
...
def generate_key_pair(secret_code):
    ...
    f = open("UserData/Keys/priv_key.txt", "wb")  # private key
    f.write(pair.exportKey(passphrase=str(secret_code)))
    f.close()

    pub_key = pair.publickey().exportKey()

    with open("UserData/Keys/pub_key.txt", 'wb') as f:  # the plain text public key for providing to ledger
        f.write(pub_key)
        f.close()
...
def read_private_key(secret_code):
    ...
    with open("UserData/Keys/priv_key.txt", 'rb') as f:
        data = f.read()
...
def read_public_key():
    assert os.path.exists('UserData/Keys'), 'No RSA keys! Please generate them with generate_key_pair()'
    with open("UserData/Keys/pub_key.txt", 'rb') as f:
        data = f.read().replace("\\n","\n")
...
def lock_n_load(plaintext, secret_code, receiver_address, receiver_key):
    ...
    keys = load_keys(get_text_keys(bounces))
...
def unlock(locked_message, secret_code):
    ...
    if needle[252:256] == '////':
        ...
        print 'Message:', needle[:252]
    else:
        print 'Message is still locked! Bouncing...'
    return needle
...
###received and authenticated. this occurs with probability 1/4 ###

generate_key_pair('swagyolo')
generate_address_pool(5)
locked = lock_n_load('attack at dawn', 'swagyolo', random_address, read_public_key())
